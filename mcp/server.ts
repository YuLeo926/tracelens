...
export function registerMcpTools(server: McpToolRegistrar, handlers: TraceLensHandlers): void {
  server.registerTool(
    "list_sessions",
    { description: TOOL_DESCRIPTION, inputSchema: z.object({ scope: z.enum(["current_project", "all"]).optional(), provider: z.enum(PROVIDERS).optional(), limit: z.number().int().min(1).max(20).optional() }).strict() },
    async (args) => callTool(() => handlers.listSessions(args as Parameters<TraceLensHandlers["listSessions"]>[0])),
  );
  server.registerTool(
    "get_session_overview",
    { description: TOOL_DESCRIPTION, inputSchema: z.object({ sessionId: sessionIdSchema() }).strict() },
  ...
  server.registerTool(
    "get_event_detail",
    { description: TOOL_DESCRIPTION, inputSchema: z.object({ sessionId: sessionIdSchema(), eventId: eventIdSchema() }).strict() },
    async (args) => callTool(() => handlers.getEventDetail(args as Parameters<TraceLensHandlers["getEventDetail"]>[0])),
  );
  server.registerTool(
    "get_viewer_link",
    { description: TOOL_DESCRIPTION, inputSchema: z.object({ sessionId: sessionIdSchema(), eventId: eventIdSchema().optional() }).strict() },
