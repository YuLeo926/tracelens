...
function FactSection({ title, rows, onOpenEvent }: { title: string; rows: FactRow[]; onOpenEvent: (eventId: string) => void }) {
  ...
  return (
    <section className="border-b border-border" aria-label={title}>
      <h2 className="px-4 py-2 text-xs font-semibold text-muted">{title}</h2>
      <div>
        {rows.map((row) => {
          const content = <><span className="evidence-text text-[13px] text-text">{row.label}</span><span className="mono evidence-text max-w-full text-[12px] text-muted sm:shrink-0">{row.value}</span></>;
          return row.eventId ? (
            <button key={row.id} type="button" onClick={() => onOpenEvent(row.eventId!)} className="flex min-h-10 w-full flex-col items-start justify-between gap-1 border-t border-border-soft px-4 py-2 text-left hover:bg-panel-2 sm:flex-row sm:items-center sm:gap-4">
              {content}
            </button>
          ) : (
...
export function SessionOverview({ session, onOpenEvent, onOpenPicker, sessionsButtonRef }: Props) {
  ...
  const runRows = rows.filter((row) => !row.id.startsWith("error-") && !row.id.startsWith("slowest-") && !row.id.startsWith("tokens-") && !row.id.startsWith("repeated-") && !totals.includes(row));

  return (
    <div className="col-span-2 flex min-h-0 flex-1 flex-col overflow-auto bg-panel">
      <header className="flex flex-wrap items-start justify-between gap-3 border-b border-border px-4 py-3">
        <div className="min-w-0">
          <h1 className="evidence-text text-sm font-semibold text-text">{sessionDisplayText(session.title, "Untitled session")}</h1>
          <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted"><span className="capitalize">{session.provider}</span><span className="evidence-text">{sessionDisplayText(session.project, "No project")}</span><span>{LIFECYCLE_LABEL[session.lifecycle]}</span></div>
        </div>
        {onOpenPicker && <button ref={sessionsButtonRef} type="button" onClick={onOpenPicker} className="shrink-0 rounded border border-border px-2.5 py-1.5 text-[12px] text-muted hover:text-text">Sessions</button>}
      </header>
      <div className="grid grid-cols-2 border-b border-border sm:grid-cols-4">
        {totals.map((row) => <div key={row.id} className="min-w-0 border-r border-border-soft px-4 py-2 last:border-r-0"><div className="text-[11px] text-muted">{row.label}</div><div className="mono evidence-text mt-0.5 text-[12px] text-text">{row.value}</div></div>)}
      </div>

      <FactSection title="Error events" rows={rowsWithPrefix(rows, "error-")} onOpenEvent={onOpenEvent} />
      {!session.facts.errorEvents.length && <p className="border-b border-border px-4 py-3 text-sm text-muted">No error events recorded.</p>}
